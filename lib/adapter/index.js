...
import {
  currentModeId, modeForPolicy, projectModes, selectionFor,
} from '../acp/selectors.js'
import { withCreditTags } from '../credit.js'
...
/**
 * Decide what to report for one turn's usage.
 *
 * An ACP agent may report every counter as zero, and at least one shipping CLI
 * (Qoder) does exactly that. Emitting that reading would hand the harness a
 * measurement it would then price as a genuinely free turn, which is a claim the
 * agent never made — it simply did not measure. The `usage` chunk is optional
 * (`BlockAssembler.usage` stays undefined until one arrives), so a zero reading
 * is omitted instead, and the baseline is still advanced so a later turn that
 * does report real numbers yields a correct delta.
 *
 * @param {{ usage: any, reading: any }} delta - the computed delta and new baseline.
 * @returns {{ usage: any, reading: any }} the delta to report, or none when it carries no information.
 */
export function reportableUsage(delta) {
  const { usage, reading } = delta
  const measured = usage.inputTokens > 0 || usage.outputTokens > 0
    || (usage.cacheReadTokens ?? 0) > 0 || (usage.cacheWriteTokens ?? 0) > 0
    || (usage.reasoningTokens ?? 0) > 0
  return measured ? { usage, reading } : { usage: undefined, reading }
}
...
export class AcpLlmAdapter extends LlmAdapter {
  ...
  async resolveModel(provider, model) {
    ...
    try {
      // Keyed by this exact model: an agent advertises levels per model, so the
      // route's whole set would attribute levels a model does not have.
      const { reasoningByModel } = await this.catalog.read(provider, () => this.#probe(provider, agent))
      const reasoning = reasoningByModel?.get(model)
   ...
   * @returns {Promise<{ models: any[], reasoningByModel: Map<string, any>, modes: any[] }>} the catalog.
   */
  async #probe(provider, agent) {
    ...
    return {
      // Tagged here, at the single point every catalog read passes through, so
      // `listModels` and `resolveModel` cannot disagree about a model's name.
      // Only the display name changes; the id is what a request carries.
      models: withCreditTags(result.models.map((model) => ({ ...model, provider })), agent.showCredit === true),
  ...
  async * stream(options) {
    ...
    try {
      if (plan.reuse && binding !== undefined) {
        ...
        created = true
        // An advertised method is not a demand; the session is attempted first
        // and only a genuine auth refusal is retried through the method.
        try {
          await connection.newSession(cwd)
        } catch (error) {
          if (error?.code !== 'ACP_AUTH_REQUIRED' || agent.authMethodId === undefined) throw error
          await connection.authenticate(agent.authMethodId)
          await connection.newSession(cwd)
        }
        ...
        await this.#select(connection, agent, options.model, options.reasoningEffort)
      }
      ...
      const { usage, reading } = response.usage === undefined || response.usage === null
        ...
        : reportableUsage(usageDelta(response.usage, previousUsage))
   ...
   * Apply the requested model and reasoning effort to a fresh ACP session.
   /**
   * Apply this session's permission mode, model, and reasoning level.
   *
   * Order matters and is not interchangeable:
   *
   *   1. **Mode first.** Permission is a session mode, not a prompt-time answer,
   *      so a client that only replies to permission requests has no say over an
   *      agent whose mode never asks. Selecting the mode is what makes the
   *      configured policy real.
   *   2. **Model next.** The model determines which reasoning levels exist at
   *      all.
   *   3. **Reasoning last, re-reading the options.** Qoder advertises
   *      `reasoning_effort` only after a model that supports it is selected, so
   *      the effort must be resolved against the options the model selection
   *      just returned. Resolving it against the pre-selection options would
   *      silently drop every level on exactly the models that have them.
   *
   * Every step is best-effort: an agent may not advertise the value, or may not
   * implement `session/set_config_option` at all. Neither justifies refusing the
   * request — the agent then runs on its own default, which is a usable turn.
   *
   * @param {AcpConnection} connection - the session's connection.
   * @param {any} agent - the agent definition, carrying the permission policy.
   ...
   */
  async #select(connection, agent, model, reasoningEffort) {
    const modes = projectModes(connection.configOptions, connection.modeState)
    const mode = modeForPolicy(
      agent.permission ?? 'reject',
      modes,
      currentModeId(connection.configOptions, connection.modeState),
    )
    await this.#apply(connection, selectionFor(connection.configOptions, 'mode', mode ?? ''))

    await this.#apply(connection, selectionFor(connection.configOptions, 'model', model), (options) => {
      // The options the agent returned with the model selection replace the
      // session's, so reasoning resolves against what that model actually has.
      connection.configOptions = options
    })

    const effort = reasoningEffort === undefined
      ? undefined
      : selectionFor(connection.configOptions, 'reasoning', reasoningEffort)
    await this.#apply(connection, effort)
  }

  /**
   * Send one `session/set_config_option`, tolerating a refusal.
   * @param {AcpConnection} connection - the session's connection.
   * @param {{ configId: string, value: string } | undefined} selection - the selection, if any.
   * @param {(configOptions: any[]) => void} [onApplied] - receives the returned options on success.
   * @returns {Promise<void>} resolves once the attempt settles.
   */
  async #apply(connection, selection, onApplied) {
    ...
    try {
      ...
      if (Array.isArray(response?.configOptions)) onApplied?.(response.configOptions)
    } catch (error) {
