...
export class AcpConnection {
   ...
   * @throws {AcpError} stage `new-session`; code `ACP_AUTH_REQUIRED` when the agent refuses for authentication.
   */
  async newSession(cwd) {
    if (this.needsAuthentication) {
      // The agent demanded auth at initialize. Without a method configured this
      // is a distinct, actionable failure — never a generic session error.
      throw new AcpError(
        `acp: this agent requires authentication (${this.authMethods.map((m) => String(/** @type {{ id?: unknown }} */ (m).id)).join(', ')})`,
        ACP_AUTH_REQUIRED,
        { stage: 'auth', authMethods: this.authMethods },
      )
    }
    try {
