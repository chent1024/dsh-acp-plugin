...
/**
 * Describe a handshake failure compactly enough for a user-facing message.
 *
 * The point is to distinguish the causes a user can act on: a transport code
 * when there is one, then the message, then the cause's own message. A timeout
 * carries no code, so it is named rather than rendered as "undefined".
 *
 * @param {unknown} error - the failure the handshake settled with.
 * @returns {string} a one-line description naming the reason.
 */
function describeCause(error) {
  if (error === null || error === undefined) return 'no reason reported'
  const coded = /** @type {{ code?: unknown, message?: unknown }} */ (error)
  const code = typeof coded.code === 'string' || typeof coded.code === 'number'
    ? `${String(coded.code)}: `
    : ''
  const message = typeof coded.message === 'string' && coded.message.length > 0
    ? coded.message
    : String(error)
  const cause = /** @type {{ cause?: { message?: unknown } }} */ (error).cause
  const nested = typeof cause?.message === 'string' && cause.message.length > 0 && cause.message !== message
    ? ` — ${cause.message}`
    : ''
  return `${code}${message.split('\n')[0].slice(0, 200)}${nested.slice(0, 200)}`
}
...
export class AcpConnection {
  ...
  static async open(spec) {
    ...
    // The seam scrubs its own parent environment, so the child's PATH must be
    // supplied here. A GUI-launched host has a minimal PATH that omits the
    // toolchain these CLIs need: `codebuddy` is a `#!/usr/bin/env node` script
    // and `qoder` looks up `qodercli` on PATH, so both failed with the agent
    // apparently missing rather than misconfigured.
    const env = await childEnvironment(process.env, spec.env ?? {})
    let child
    try {
      child = spec.spawn({
    ...
    try {
      ...
      // The wrapper must carry the underlying reason into its message: the
      // panel shows the message, so a lost cause leaves the user with
      // "did not complete initialization" and no way to tell a missing binary
      // from a rejected protocol version from a crash mid-handshake.
      throw new AcpError(
        `acp: the agent did not complete initialization (${describeCause(error)})`,
        'ACP_INITIALIZE_FAILED',
        { stage: 'initialize', cause: error },
      )
    }
  }
