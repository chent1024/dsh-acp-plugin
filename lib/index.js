...
/**
 * Services this plugin reads directly.
 *
 * `subprocess` and `sessions` are required, not optional: every ACP agent is a
 * child process started through the subprocess seam, and a request's working
 * directory comes from its session. Reading either without declaring it throws
 * at the point of use — the property proxy refuses an undeclared service — so
 * leaving them out would surface as a failure on the first model call rather
 * than as a composition error.
 *
 * `settings`, `subagents`, and `connection` are deliberately absent: each is
 * reached through `ctx.inject` or `ctx.get`, so a composition without one
...
export function apply(ctx, config) {
  ...
  registerPanel(ctx, {
    ...
    probe: async (key) => {
      ...
      // Reasoning is reported per model, because an agent advertises levels per
      // model; the card shows each model's own set.
      return {
        models: result.models,
        modes: result.modes,
        reasoningByModel: Object.fromEntries(result.reasoningByModel ?? []),
      }
    },
    logger: ctx.logger,
  })
