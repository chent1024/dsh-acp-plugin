...
| `showCredit` | `true` | 把 agent 披露的倍率追加到模型名后，例如 `Ultimate  [2x]`、`Qwen3.8-Flash  [FREE]`。见[倍率](#倍率)。 |
...
因此这段文本在选择器里看不见。打标签默认开启，会把倍率追加到模型名后：
...
模型，其名称原样保留 —— 包括 WorkBuddy 的全部模型，它完全不发送 description。

卡片上没有这个开关：模型页不为一个没人需要关掉的行为提供开关。若要让某个 agent 保持原样
（例如名称还会出现在别处，倍率在那里读起来像噪音），直接改 profile 的
`cordis.patch.yml` 里的 `showCredit: false`。
